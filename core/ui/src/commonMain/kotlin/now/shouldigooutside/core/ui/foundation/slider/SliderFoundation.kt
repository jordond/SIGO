package now.shouldigooutside.core.ui.foundation.slider
...
import now.shouldigooutside.core.ui.foundation.IncreaseHorizontalSemanticsBounds
import now.shouldigooutside.core.ui.foundation.slider.SliderFoundationDefaults.SliderRangeTolerance
import now.shouldigooutside.core.ui.foundation.slider.SliderFoundationDefaults.ThumbHeight
import now.shouldigooutside.core.ui.foundation.slider.SliderFoundationDefaults.ThumbShape
import now.shouldigooutside.core.ui.foundation.slider.SliderFoundationDefaults.ThumbSizeOnPress
import now.shouldigooutside.core.ui.foundation.slider.SliderFoundationDefaults.ThumbTrackGap
import now.shouldigooutside.core.ui.foundation.slider.SliderFoundationDefaults.ThumbWidth
import now.shouldigooutside.core.ui.foundation.slider.SliderFoundationDefaults.TrackHeight
import now.shouldigooutside.core.ui.foundation.slider.SliderFoundationDefaults.TrackInsideCornerSize
import now.shouldigooutside.core.ui.foundation.slider.SliderFoundationDefaults.TrackTickSize
    ...
    Layout(
        content = {
            Box(
                modifier = Modifier
                    ...
                    .wrapContentWidth()
                    .onSizeChanged { size ->
                        state.thumbWidth = size.width.toFloat()
                    },
            ) {
                thumb(state)
            }
        ...
        modifier = modifier
            ...
            .slideOnKeyEvents(
                ...
                value = state.value,
                isRtl = state.isRtl,
                onValueChangeState = state.onValueChange,
                onValueChangeFinishedState = state.onValueChangeFinished,
            ).then(press)
            .then(drag),
...
private fun RangeSliderComponent(
    ...
    Layout(
        ...
        val startThumbPlaceable =
            measurables.fastFirst { it.layoutId == RangeSliderComponents.STARTTHUMB }.measure(constraints)

        val endThumbPlaceable =
            measurables.fastFirst { it.layoutId == RangeSliderComponents.ENDTHUMB }.measure(constraints)
        ...
        val endThumbOffsetX =
            (trackPlaceable.width * state.coercedActiveRangeEndAsFraction + endCorrection).roundToInt()
        val trackOffsetY = (sliderHeight - trackPlaceable.height) / 2
...
internal object SliderFoundationDefaults {
    ...
    internal val TrackTickSize = 4.dp
    internal val ThumbShape = RoundedCornerShape(50)
    internal const val SliderRangeTolerance = 0.0001
...
@Immutable
@JvmInline
public value class SliderRange(
    public val packedValue: Long,
) {
    @Stable
    ...
    public companion object {
        @Stable
        public val Unspecified: SliderRange = SliderRange(Float.NaN, Float.NaN)
    }
...
internal fun SliderRange(range: ClosedFloatingPointRange<Float>): SliderRange {
    ...
    val isUnspecified = start.isNaN() && endInclusive.isNaN()
    require(isUnspecified || start <= endInclusive + SliderRangeTolerance) {
        "ClosedFloatingPointRange<Float>.start($start) must " +
            "be <= ClosedFloatingPoint.endInclusive($endInclusive)"
    }
    return SliderRange(packFloats(start, endInclusive))
}
